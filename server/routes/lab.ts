...
// POST /api/lab/orders
router.post("/orders", requireRole("admin", "doctor", "nurse", "lab_tech"), async (req: AuthRequest, res) => {
  try {
    const labId = await nextLabId(req.user!.tenantId);
    const order = await LabOrder.create({ ...req.body, tenantId: req.user!.tenantId, labId });
