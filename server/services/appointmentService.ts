...
export async function listAppointments(tenantId: string, filters: AppointmentListFilters) {
  const { date, status, type, doctor, patientId, page = "1", limit = "50" } = filters;
...
export async function getQueue(tenantId: string, filters: { doctor?: string; date?: string }) {
  ...
  return raw.sort((a, b) => (ORDER[a.status] ?? 9) - (ORDER[b.status] ?? 9));
}

export async function getSlots(tenantId: string, doctor: string, date: string) {
  if (!doctor || !date) throw AppError.badRequest("doctor and date required");
...
export async function createAppointment(tenantId: string, body: Record<string, any>) {
  const aptId = await nextAptId(tenantId);
  const { token: _t, tokenNumber: _tn, ...rest } = body;

  const { token, tokenNumber } = await generateToken(tenantId, rest.doctor, rest.department, rest.date);
...
export async function getAppointment(tenantId: string, id: string) {
  const appt = await Appointment.findOne({ _id: id, tenantId });
  if (!appt) throw AppError.notFound("Appointment not found");
  return appt;
}

export async function updateAppointment(tenantId: string, userName: string, id: string, body: Record<string, any>) {
  const { token: _t, tokenNumber: _tn, ...updates } = body;

  const before = await Appointment.findOne({ _id: id, tenantId });
  if (!before) throw AppError.notFound("Appointment not found");
...
export async function checkin(tenantId: string, id: string) {
  ...
  if (!appt) throw AppError.notFound("Appointment not found or already checked in");
  return appt;
}

export async function callPatient(tenantId: string, id: string) {
  const target = await Appointment.findOne({ _id: id, tenantId });
  if (!target) throw AppError.notFound("Appointment not found");
  ...
  const appt = await Appointment.findOneAndUpdate(
    { _id: id, tenantId, status: "Waiting" },
    { $set: { status: "In Consult", calledAt: new Date() } },
    { new: true }
  );
  if (!appt) throw AppError.badRequest("Patient must be in Waiting status to be called");
  return appt;
}
